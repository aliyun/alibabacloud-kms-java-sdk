...
客户端机制
----------
阿里云KMS Java SDK支持调用KMS共享网关和KMS实例网关提供的API。

阿里云KMS Java SDK默认情况下会将以下API的请求发送到KMS实例网关，其它KMS API则发送到KMS共享网关。
...
阿里云KMS Java SDK也支持将以上的API请求发送到KMS共享网关，具体方法请参考使用示例-特殊使用场景。

使用示例
----------
### 常规使用场景
#### 场景一 可以参考下面的代码同时调用KMS共享网关和KMS实例网关的API。
```Java
...
```
### 特殊使用场景
#### 场景一 参考如下代码将上述所有API的调用请求发送到KMS共享网关。
```Java
